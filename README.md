...
- Open an existing show from its `name.tar.gz` (the `name.sho` is optional — it is rebuilt from the
  archive), start a new empty show (a console-saved empty show), or start from your own console blank template.
...
- Generate the show again (`.sho` + `.tar.gz`, optionally zipped). Show names are console-safe:
  letters and digits only, at most 5 characters (`name.sho`), lower case.
...
- **The empty show** (`public/blank/blank.*`) is an empty show saved on a grandMA1 console (v6.801)
  with every user profile except DEFAULT removed. It is not synthesized: the console writes its empty
  pools at full size (999 empty preset/group/cue slots, worlds, pages, timecode, effect forms …), and
  an app-generated empty show crashed the console. A patch built on it loads on the console.
